...
@hydra.main('configs', config_name='default') #Quindi quando viene richiamato hydra questo installa le configurazioni di default da configs. Bisogna passare il parametro DictConfig alla funzione successiva
def train(cfg: DictConfig):  
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=cfg.data.batch_size, shuffle=True) #Utile per creare minibatch, tenere il batch_size basso se non si dispone di GPU
    ...
    trainer = pl.Trainer(**cfg.trainer) # Visto che vuole la lista chiave/valore si mettono ** davanti così da convertire
